...
define( "PMPRO_ETS_STRIPE_API_VERSION", "2022-11-15" );
...
class PMProGateway_etsStripe extends PMProGateway {
	...
	public static function init() {
		...
		add_action( 'update_option_pmpro_stripe_payment_flow', array( 'PMProGateway_etsStripe', 'update_option_pmpro_stripe_payment_flow' ), 10, 1 );
		add_action( 'pmpro_payment_option_fields', array( 'PMProGateway_etsStripe', 'show_set_up_webhooks_popup' ) );
        ...
        */
		...
		if ( ( $default_gateway == "etsstripe" || $current_gateway == "etsstripe" ) && empty( $_REQUEST['review'] ) ) {
			...
			if ( ! self::using_stripe_checkout() ) {
				...
				add_filter('pmpro_include_payment_information_fields', array('PMProGateway_etsStripe', 'show_stripe_checkout_pending_warning'));
				add_filter('pmpro_checkout_before_change_membership_level', array('PMProGateway_etsStripe', 'pmpro_checkout_before_change_membership_level'), 10, 2);
				add_filter('pmprommpu_gateway_supports_multiple_level_checkout', '__return_false', 10, 2);
				add_action( 'pmpro_billing_preheader', array( 'PMProGateway_etsStripe', 'pmpro_billing_preheader_stripe_checkout' ) );
			}
		}
		...
		add_filter( 'pmpro_process_refund_stripe', array( 'PMProGateway_etsStripe', 'process_refund' ), 10, 2 );
	}
	...
	public static function pmpro_payment_option_fields( $values, $gateway ) {
		...
		<tr class="gateway pmpro_stripe_legacy_keys <?php if ($stripe->show_legacy_keys_settings() ) { echo 'gateway_etsstripe'; } ?>" <?php if ( $gateway != "etsstripe" || ! $stripe->show_legacy_keys_settings() ) { ?>style="display: none;"<?php } ?>>
		...
		<tr class="pmpro_settings_divider gateway gateway_etsstripe_<?php echo esc_attr( $stripe->gateway_environment ); ?>" <?php if ( $gateway != "etsstripe" ) { ?>style="display: none;"<?php } ?>>
			<td colspan="2">
				<hr />
				<h2><?php esc_html_e( 'Webhook Status', 'paid-memberships-pro' ); ?></h2>
			</td>
		</tr>
		<tr class="gateway gateway_etsstripe_<?php echo esc_attr( $stripe->gateway_environment ); ?>" <?php if ( $gateway != "etsstripe" ) { ?>style="display: none;"<?php } ?>>
	      <td colspan="2">
				<?php
		...
		<tr class="gateway gateway_etsstripe" <?php if ( $gateway != "etsstripe" ) { ?>style="display: none;"<?php } ?>>
			...
			<td><code><?php echo esc_html( PMPRO_ETS_STRIPE_API_VERSION ); ?></code></td>
		</tr>
		<tr class="gateway gateway_etsstripe" <?php if ( $gateway != "etsstripe" ) { ?>style="display: none;"<?php } ?>>
			<th scope="row" valign="top">
		...
		<tr class="gateway gateway_etsstripe gateway_etsstripe_checkout_fields" <?php if ( $gateway != "etsstripe"  ) { ?>style="display: none;"<?php } ?>>
			<th scope="row" valign="top">
	...
	public static function pmpro_required_billing_fields( $fields ) {
		...
		if ( $pmpro_stripe_lite ) {
			$remove = array_merge( $remove, [ 'bfirstname', 'blastname', 'baddress', 'baddress1', 'bcity', 'bstate', 'bzipcode', 'bphone', 'bcountry', 'CardType' ] );
		}
	...
	public static function pmpro_checkout_order( $morder ) {
		...
		//pm_1PdRspEJfNYkxEoIf6BDlLZD
		    error_log("The user ID is: " . $morder->user_id);
		...
		if ( ! empty ( $_REQUEST['payment_method_id'] ) ) {
			...
			update_pmpro_membership_order_meta( $morder->id, '_ets_stripe_payment_method_id', sanitize_text_field( $_REQUEST['payment_method_id'] ) );
			update_user_meta( $morder->user_id, '_ets_stripe_payment_method_id', sanitize_text_field( $_REQUEST['payment_method_id'] ) );
   ...
   /**
	...
	public static function stripe_connect_save_options() {
		...
		update_option( 'pmpro_gateway', 'etsstripe' );
	...
	static function show_stripe_checkout_pending_warning($show)
	{
		...
		?>
		<span id="pmpro_payment_information_fields" <?php if( $gateway != "etsstripe" ) { ?>style="display: none;"<?php } ?>>
			<?php
			// If the current user's last order is a pending Stripe order, warn them that they already have a pending order.
			$last_order = new MemberOrder();
			$last_order->getLastMemberOrder( get_current_user_id(), null, null, 'etsstripe' );
	...
	static function pmpro_checkout_before_change_membership_level($user_id, $morder)
	{
		...
		if ( empty( $morder ) || $morder->gateway != 'etsstripe' ) {
			return;
		}
		...
		if ( ! empty( $subscription_data ) ) {
			...
			$checkout_session_params['subscription_data']['description'] = PMProGateway_etsStripe::get_order_description( $morder );
		} elseif ( ! empty( $payment_intent_data ) ) {
			$checkout_session_params['payment_intent_data'] = $payment_intent_data;
			$checkout_session_params['payment_intent_data']['description'] = PMProGateway_etsStripe::get_order_description( $morder );
		}
	...
	public static function pmpro_billing_preheader_stripe_checkout() {
		...
		if ( empty( $pmpro_billing_subscription ) || 'etsstripe' !== $pmpro_billing_subscription->get_gateway() ) {
			return;
		}
		...
		add_action( 'pmpro_billing_preheader', array( 'PMProGateway_etsStripe', 'pmpro_checkout_after_preheader' ), 15 );
		add_filter( 'pmpro_billing_order', array( 'PMProGateway_etsStripe', 'pmpro_checkout_order' ), 15 );
		add_filter( 'pmpro_include_payment_information_fields', array(
			'PMProGateway_etsStripe',
			'pmpro_include_payment_information_fields'
		), 15 );
	...
	public static function pmpro_billing_preheader_stripe_customer_portal() {
		...
		if ( empty( $pmpro_billing_subscription->get_gateway() ) || 'etsstripe' !== $pmpro_billing_subscription->get_gateway()  ) {
			return;
		}
	...
	private function show_connect_payment_option_fields( $livemode, $values, $gateway ) {
		...
		?>
		<tr class="pmpro_settings_divider gateway gateway_etsstripe_<?php echo esc_attr( $environment ); ?>"
		...
		<tr class="gateway gateway_etsstripe_<?php echo esc_attr( $environment ); ?>" <?php if ( $gateway != "etsstripe" || $gateway_environment != $environment ) { ?>style="display: none;"<?php } ?>>
            <th scope="row" valign="top">
								...
								if ( self::has_connect_credentials( $environment ) ) {
									$connect_url = add_query_arg(
										array(
											...
											'return_url' => rawurlencode( add_query_arg( array( 'page' => 'pmpro-paymentsettings', 'edit_gateway' => 'etsstripe', 'pmpro_stripe_connect_deauthorize_nonce' => wp_create_nonce( 'pmpro_stripe_connect_deauthorize_nonce' ) ), admin_url( 'admin.php' ) ) ),
										),
										$connect_url_base
									);
									?>
									...
									$connect_url = add_query_arg(
										array(
											'action' => 'authorize',
											'gateway_environment' => $environment2,
											'return_url' => rawurlencode( add_query_arg( array( 'page' => 'pmpro-paymentsettings', 'edit_gateway' => 'etsstripe', 'pmpro_stripe_connect_nonce' => wp_create_nonce( 'pmpro_stripe_connect_nonce' ) ), admin_url( 'admin.php' ) ) ),
										),
										$connect_url_base
									);
									?>
   ...
   *
	...
	public function create_subscription_for_customer_from_order( $customer_id, $order ) {
		...
		if ( empty( $order->payment_method_id ) ) {
			...
			//return false;
			//$payment_method_id = get_pmpro_membership_order_meta($order->id, '_ets_stripe_payment_method_id', true);
			$payment_method_id = get_user_meta($order->user_id, '_ets_stripe_payment_method_id', true);
		}
		else {
